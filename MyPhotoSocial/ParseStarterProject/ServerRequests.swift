...
class ServerRequests : CanFailProtocol
{
    var users = [User]()
...
extension User where Self: FollowingUser
{
...
typealias PhotoCardUser = protocol<User, PhotoCard>

struct PhotoCardModel : PhotoCardUser
{
    let uniqueID: String
    let username: String
    let message: String?
...
extension User where Self:PhotoCardUser
{
    var isFollowing: Bool { return true }
}
